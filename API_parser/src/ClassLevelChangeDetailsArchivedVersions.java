...
public class ClassLevelChangeDetailsArchivedVersions {
	...
	public static void main(String[] args) throws IOException {
		...
		String toWrite = "";
		File file = new File("output/ClassesChangesArchivedVersions.csv");
		...
		for (int version = 18; version >= 3; version--) {
			...
			File changedClassesFile = new File("./input/" + versionNum + "/changes/classes_index_changes.html");
			Document changedClassesDoc = Jsoup.parse(changedClassesFile, "UTF-8", "");
			...
			for (Element changedClass : changedClassesList) {
				...
				File changedClassFile = new File("./input/" + versionNum + "/changes/" + changedClassLink);
				Document changedClassContent = Jsoup.parse(changedClassFile, "UTF-8", "");
				...
				if (count == 0) {
					...
					// Case 7: Keyword related change (Final, Abstract, Visibility, Static)
					else if (text.contains("final") || text.contains("abstract") || text.contains("visibility") || text.contains("static")){
						String changedElementModificationType = text.substring(0, text.indexOf(" "));
						String changedElementType = "Keyword";
						String changedElementName = text.substring(text.indexOf("from")); 
						...
						toWrite = versionNum  + ",Class,\""+ changedClassName + "\",Changes," + changedElementType + ",\"" + whitespaceReplacer(changedElementName) + "\"," + changedElementModificationType + "\n";
						bw.write(toWrite);
					}
					// Case 8: Unexpected input
					else {
						System.out.println("\tCase 8 Unexpected Input:\t" + changedClassName + "\t" + text);
					}
				}
			...
			/* Loads HTML and grabs table entries from list */
			File addedClassesFile = new File("./input/" + versionNum + "/changes/classes_index_additions.html");
			Document addedClassesDoc = Jsoup.parse(addedClassesFile, "UTF-8", "");
			...
			/* Loads HTML and grabs table entries from list */
			File removedClassesFile = new File("./input/" + versionNum + "/changes/classes_index_removals.html");
			Document removedClassesDoc = Jsoup.parse(removedClassesFile, "UTF-8", "");
